...
- [Chrome proof video](./issue-46-element-shadows.webm)
...
| Image frame silhouette | Worker-export alpha bounds prove the rectangular frame shadow; the Chrome proof uses transparent image content with an 8 pt border. |
...
| Text glyph and underline silhouette | A transparent-PNG assertion proves the shadow follows the underlined text rather than the text box. |
...
Results: lint and build passed; the full Playwright suite passed with 133 tests and one intentionally skipped proof-recorder test; the explicit Chrome proof run passed.
