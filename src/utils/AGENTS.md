...
### Orientation & Transform Logic (`orientationMatrix.ts`, `propertyUtils.ts`)
Manages non-destructive image manipulation (pan, zoom, flip, rotate) and property editing math.
...
- **Property math**:
    - Unit conversions for inch/cm fields.
    - Box resize/reposition conversion between UI units and normalized box.
    - Zoom update logic that keeps pan bounds valid.
...
- Also controls zoom-compensated image UI controls (pan control and low-res badge sizing).
...
- **Editor/Export Parity**: If image/text render logic changes in one renderer, validate and update the other renderer accordingly.
